...
class CardEditProject extends Component {
    ...
    render() {
        ...
        this.state.alldataOwns.forEach((el, index) => {
            ownsList.push(<div key={`href${counter++}`}>
              <a href="#" key={`href${counter++}link1`}>&laquo;{el.item.fields.name}&raquo; </a>
              <a href="#" key={`href${counter++}link2`} className="card-link text-danger"><span className="oi oi-trash" onClick={() => this.dropOwn(index)} /></a>
            </div>);
        });
